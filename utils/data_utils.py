...
def get_part_of_dataset(dataset, ratio, is_random=True):
    if is_random:
...
@dataclass
class DataCollatorHubertSeq2SeqWithPadding:
    processor: Any
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        batch = {
            'input_features': torch.stack(
                [torch.tensor(feature["eeg_raw"][0], dtype=torch.float32) for feature in features])
        }
        # get the tokenized label sequences
        # add same in the last code, because the original one is missing one element
        label_features = [{"input_ids": feature["hubert_codes"]+[feature["hubert_codes"][-1]]} for feature in features]
...
import time
@dataclass
class DataCollatorBrainSpeechSeq2SeqWithPadding:
    processor: Any
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        #  新增的 subject_index，mel_spec
        subject_index = torch.stack([torch.tensor(feature['subj'], dtype=torch.int) for feature in features])
        # print('subject_index')
        mel_spec = torch.stack(
            [torch.tensor(feature['speech_mel_input_features'][0], dtype=torch.float32) for feature in features])
        # print('mel_spec')
        ...
        batch["labels"] = labels
        # t3=time.time()
        # print('data_utils done',round(t3-t1,2))
        # for k,v in batch.items():
        #     if k is not 'useful_length':
        #         print(k,v.shape)
        return batch
